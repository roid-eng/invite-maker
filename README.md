...
## Firestore TTL 정책 설정 (자동 만료 삭제)

만료된 초대장 문서를 Firestore가 자동 삭제하도록 TTL 정책을 활성화합니다.

1. Firebase 콘솔 → **Firestore Database** → 상단 탭 **색인(Indexes)**
2. **TTL 정책** 탭 → **TTL 정책 추가** 클릭
3. 아래와 같이 입력 후 저장
...
> TTL 삭제는 Firestore 내부 배치 작업으로 처리되며 정확한 삭제 시각은 보장되지 않습니다.
> 앱 레벨 만료 체크는 TTL과 별개로 각 페이지(`/invite`, `/preview`, `/manage`)에서 수행합니다.
